...
import { IEvent,ISession } from './IEvent.model'
...
export class EventService {
  ...
  private extractEvent(res:Response){
    let body = res.json();
  ...
  searchSession(sessionTerm:string){
    ...
    EVENTS.forEach(event => {
      var matchedSession = event.sessions.filter(session =>
        session.name.toLocaleLowerCase().indexOf(sessionTerm.toLocaleLowerCase()) > -1)
