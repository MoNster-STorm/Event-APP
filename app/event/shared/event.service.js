...
var EventService = (function () {
    ...
    EventService.prototype.getEvents = function () {
        // return this.http.get('http://localhost:3000/events').map(this.extractEvent).catch(this.handleError);
        return EVENTS;
    };
    EventService.prototype.getEvent = function (id) {
        var eventfound = EVENTS.find(function (event) { return event.id === id; });
        console.log(eventfound);
        return eventfound;
    };
    EventService.prototype.extractEvent = function (res) {
        var body = res.json();
        console.log("extracted" + body);
        return body.data || {};
    };
    EventService.prototype.updateEvent = function (event) {
        var eventIndex = EVENTS.findIndex(function (e) { return e.id = event.id; });
        EVENTS[eventIndex] = event;
        console.log(EVENTS[eventIndex]);
        console.log(EVENTS);
    };
    EventService.prototype.searchSession = function (sessionTerm) {
        var results = [];
        EVENTS.forEach(function (event) {
            var matchedSession = event.sessions.filter(function (session) {
                return session.name.toLocaleLowerCase().indexOf(sessionTerm.toLocaleLowerCase()) > -1;
            });
            matchedSession = matchedSession.map(function (session) {
